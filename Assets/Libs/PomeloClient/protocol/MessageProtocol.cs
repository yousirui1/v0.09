...
namespace Pomelo.DotNetClient
{
    public class MessageProtocol
    {
        ...
        public byte[] encode(string route, uint id, JsonObject msg)
        {
            ...
            if (id > 0)
            {
				Log.d<MessageProtocol>("reqID 1");
                byte[] bytes = Protobuf.Encoder.encodeUInt32(id);

                writeBytes(bytes, offset, head);
                flag |= ((byte)MessageType.MSG_REQUEST) << 1;
                offset += bytes.Length;
				Log.d<MessageProtocol>("head: " + flag + ", " + BitConverter.ToString(head));
				Log.d<MessageProtocol>("reqID 1: " + id + ", " + bytes);
            }
            else
            {
				Log.d<MessageProtocol>("reqID 2");
            ...
            {
				Log.d<MessageProtocol>("Compress head 1");
            ...
            head[0] = flag;
			Log.d<MessageProtocol>("head: " + BitConverter.ToString(head));
            ...
            if (encodeProtos.ContainsKey(route))
            {
				...
				Log.d<MessageProtocol>(route + " encode use proto: " + BitConverter.ToString(body));
            }
            else
            {
				Log.d<MessageProtocol>(route + " encode not use proto, use raw string bytes");
